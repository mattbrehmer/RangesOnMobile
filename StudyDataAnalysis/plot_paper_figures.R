...
RT_t_diff_min.df$ratio <- RT_t_diff_min.df$facet 
RT_t_diff_min.df$ratio <- revalue(RT_t_diff_min.df$ratio, c("Start of Range" = "Start",
                                                            "End of Range" = "End",
                                                            "Length of Range" = "Length"
))

RT_t_diff_min.df$facet <- RT_t_diff_min.df$datatype
RT_t_diff_min.df[nrow(RT_t_diff_min.df) + 1,] <- list("T2-RV","Length",0,0,0,0,"Sleep","Sleep")
RT_t_diff_min.df[nrow(RT_t_diff_min.df) + 1,] <- list("T2-RV","Length",0,0,0,0,"Temperature","Temperature")
RT_t_diff_min.df[nrow(RT_t_diff_min.df) + 1,] <- list("T4-CV","Length",0,0,0,0,"Sleep","Sleep")
RT_t_diff_min.df[nrow(RT_t_diff_min.df) + 1,] <- list("T4-CV","Length",0,0,0,0,"Temperature","Temperature")
RT_t_diff_min.df <- RT_t_diff_min.df[with(RT_t_diff_min.df, order(facet,task_name)),]


plot_RT_t_diff_min <- dualChart(RT_t_diff_min.df,RT_t_diff_min.df$ratio ,nbTechs = 3, ymin = 0.925, ymax = 1.525, "", "Trial Completion Time Ratios. Error Bars, 95% CIs", 1, displayXLabels = T, displayYLabels = T, percentScale = F, displayFacetLabel = T)

ggsave(plot = plot_RT_t_diff_min, filename = "plot_RT_t_diff_min.pdf", device ="pdf", width = 3.75, height = 1.75, units = "in", dpi = 300)
...
plot_err_rate_t_diff_min <- dualChart(err_rate_t_diff_min.df,err_rate_t_diff_min.df$ratio ,nbTechs = 1, ymin = -0.15, ymax = 0.15, "", "", 0, displayXLabels = T, displayYLabels = F, percentScale = T, displayFacetLabel = T)
